...
Five years after the initial COVID-19 outbreak, we aim to investigate and compare the trends in the annual number of published works in medicine and fields in the humanities---specifically history, philosophy, art, and literature---before and after the pandemic. We also aim to compare more generally the number of published works in the humanities with those in medicine. Our analysis will highlight any potential difference in research productivity between medicine and the humanities or general patterns of decline in research pre and post-pandemic. By doing so, we hope to call attention to the importance of the humanities and, more broadly, research in academia.
...
It is clear from the scatterplot above that all four fields in the humanities---history, philosophy, art, and literature---display an increasing trend in number of published works from 2000 to 2020 and a decreasing trend thereafter.
...
ggplot(diff_table,aes(x=field1,y=field2,fill=diff)) + geom_tile(color="white") + geom_text(aes(label=round(diff)),color="black",size=4) + scale_fill_gradient(low="lightblue",high="mediumpurple",name="Mean Difference") + labs(x=NULL,y=NULL,title="Pairwise Mean Differences in Annual Number of Published Works (2000-2024)")+ theme(plot.title = element_text(hjust = 0.5))
```

---

The variation in annual number of published works from 2000 to 2024 is much higher for medicine than the four fields in the humanities, reflecting its steeper upward trend in publication output over time. 
...
1. Conduct an interrupted time-series (ITS) analysis on the annual number of published works before and after the onset of the COVID-19 pandemic in 2020 for each of the five research fields---history, philosophy, art, literature, and medicine---using multiple segmented linear regression models.

2. Perform Chow's Test to evaluate whether the year 2020, marking first outbreak of the COVID-19 pandemic, corresponds to a statistically significant shift in the parameters of the previously constructed segmented linear regressions. 

## ITS Analysis

---

The data for each of the five research fields will be split into two groups, data prior to and at the start of the COVID-19 pandemic (years 2000 to 2020) and after the pandemic's initial outbreak (years 2021 to 2024). Five segmented linear regressions will be constructed each with a singular structural break point at the year 2020. This will result in five separate piecewise functions for each field with the independent variable $\small t$, measured in years since 2000, and dependent variable $\small \hat{y}(t)$, the estimated number of works published in that field at time $\small t$. 

The segmented linear regression model constructs a linear regression using the method of ordinary least squares for points before and after a number of specified break points while ensuring continuity between regression segments at the break point. This ensures that the two separate linear regression functions which are constructed for $\small t\leq20$ and $\small t>20$ meet at $\small t=20$ exactly. A segmented linear regression model for the number of of works $\small Y_i$ published at a time $\small t_i$, measured in years since 2000 is formulated as follows:
...
All four fields in the humanities display a strong increasing linear trend in annual number of published works from the years 2000 to 2020 and, starting from 2021 onward, decrease linearly at a rate higher than their previous increase. 

Medicine also displays a strong increasing linear trend from the years 2000 to 2020 and starts to decrease starting from 2021 onward but at a rate much lower than its previous increase.
...
It is important to note that these extrapolated values are tentative estimations made under the strong assumption that the current downward trend in the annual number of published works in each field will continue.
...
The high adjusted $\small r^2$ values ($\small r^2>0.9)$ indicate that a large proportion of variation in the annual number of published works was explained by the segmented linear regressions constructed. This indicates that the segmented regressions for all five fields provided a good fit to the data. 
...
3. Medicine was the most productive field of research among the five both before and after the pandemic, producing more published works per year, on average, than all four fields in the humanities.

4. The annual number of published works from 2000 to 2024 is better modeled using a segmented linear regression with a break point at the year 2020 than a singular continuous linear regression for all five fields. 
