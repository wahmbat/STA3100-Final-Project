...
2. Perform Chow's Test to evaluate whether the presumed structural break point---the COVID-19 pandemic in 2020---corresponds to a statistically significant shift in the parameters of the previously constructed segmented regression model. 
...
All four fields in the humanities display a strong increasing linear trend in number of published works from the years 2000 to 2020 and, starting from 2021 onward, decrease linearly at a higher rate their previous increase. 
...
The high adjusted $r^2$ values ($r^2>0.9)$ indicate that a large proportion of variation in the number of published works each year was explained by the segmented regression models constructed. 
...
list_result <- lapply(list_result, function(x) sapply(x, function(y) format(y, digits = 4)))
frame_result <- data.frame(names=c("History","Philosophy","Art","Literature","Medicine"),ts=sapply(list_result,`[`,1),p=sapply(list_result,`[`,2))
